"""Offline tests for the media stream invoker: the consume bridge, error
...
def test_backpressure_all_is_lossless():
    inv = MediaInvoker(backends=[_FakeBackend(count=5)], max_queue=2, backpressure="all")
...
def test_track_selects_audio():
    ...
    inv = MediaInvoker(backends=[fake], backpressure="all")
...
def test_bad_track_rejected():
    inv = MediaInvoker(backends=[_FakeBackend()])
    with pytest.raises(ValueError, match="track"):
        asyncio.run(_collect(inv, _form(), track="subtitles"))


def test_backpressure_latest_sheds_when_consumer_lags():
    inv = MediaInvoker(backends=[_FakeBackend(count=100)], max_queue=2, backpressure="latest")
...
def test_bad_backpressure_rejected():
    with pytest.raises(ValueError, match="backpressure"):
        MediaInvoker(backends=[_FakeBackend()], backpressure="nope")
...
def test_backend_error_propagates():
    ...
    inv = MediaInvoker(backends=[_FakeBackend(count=10, raise_at=3)])
...
def test_early_break_sets_stop():
    ...
    inv = MediaInvoker(backends=[fake], max_queue=2)
...
def test_no_backend_raises():
    ...
    inv = MediaInvoker(backends=[_Picky()])
...
def test_publish_drains_all_frames():
    be = _WriterBackend()
    inv = MediaInvoker(backends=[be], max_queue=2)
...
def test_publish_bad_track_rejected():
    inv = MediaInvoker(backends=[_WriterBackend()])
    with pytest.raises(ValueError, match="track"):
